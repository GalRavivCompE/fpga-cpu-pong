# Complete candidate ISA, revision 0.2
...
The goal is a small, general-purpose 32-bit CPU that is approachable to implement on an FPGA and pleasant enough to program by hand. Current names and behavior are recorded here; binary encoding remains open. Inspiration and comparison: [RISC-V RV32I](https://docs.riscv.org/reference/isa/v20240411/unpriv/rv32.html) and [Astro8](https://sam-astro.github.io/Astro8-Computer/docs/Architecture/Instruction%20Set.html). Similar basic arithmetic is useful; counted conditional blocks, `GHOST`, `REPEAT`, the current eight-register candidate, and separate program/data spaces make this design meaningfully different. Sixteen registers remain an open alternative.

## Programmer-visible machine

- The current candidate has eight writable 32-bit registers named `A`–`H`; 16 registers are under consideration. There is no hardwired constant register. These letters select registers; they are not memory addresses. Reset clears them and sets the 32-bit program counter (PC) to byte address 0. Any register can hold a number, address, or bit pattern; there are no type tags.
...
**Operand forms.** Binary register arithmetic (`ADD`, `SUB`, `MUL`, `DIV`, `MOD`), binary bitwise logic (`AND`, `OR`, `XOR`), and variable-distance shifts use `INSTRUCTION dst, src1, src2`. Both sources are read before `dst` is written, even if `dst` names one of the sources. `NOT` uses `INSTRUCTION dst, src1`; `COUNT` uses `COUNT dst`. `ADD_CONST dst, src1, signed_imm16` uses a number encoded in the instruction instead of a second source register. Conditions use the explicit form `IF_* src1, src2, N`: they read two registers, and `N` is an instruction count, **not a third register**. There is no implicit-count two-operand form. `GHOST N` and `REPEAT N` encode numeric counts; assembler labels can resolve to those numbers. `GHOST` does not read a register count in v1.
...
| Shifts | `LSHIFT dst, src1, src2`; `RSHIFT dst, src1, src2`; `RSHIFT_SIGN dst, src1, src2` | Shift by the unsigned 32-bit distance in `src2`. `LSHIFT` and `RSHIFT` fill with zero; `RSHIFT_SIGN` copies the old top bit into open positions. A zero distance copies `src1`. A distance of 32 or more gives zero for the zero-filling operations and all sign bits for `RSHIFT_SIGN`. Hardware may compute the result at once or over several cycles. |
...
| Flow | `GHOST N`; `REPEAT N`; `LEAP target_reg`; `CALL target_reg, link_reg`; `COUNT dst` | `GHOST` unconditionally skips N next instructions: `PC=P+4(N+1)`. `REPEAT` goes to `P-4N`; N must be at least 1. `LEAP` loads the PC from a register. `CALL` first reads the old target, then writes `P+4` to `link_reg`, then jumps to that target. Return with `LEAP link_reg`. `COUNT` writes the current instruction's byte address P to `dst` and then advances normally. |
...
Yes in principle for the current eight-register candidate. A 6-bit opcode supports up to 64 operations. Three register identifiers consume 9 bits, and a 16-bit immediate/count consumes 16 bits, leaving one bit (`6+9+16+1=32`). The table has fewer than 64 operation names. Different instructions use different subsets of those fields, and unused fields can be required to equal zero. The **exact bit layout and opcode numbers remain unassigned** pending review. Sixteen registers would use 4-bit identifiers; formats with three register fields and formats with 16-bit immediates need not have the same field layout. Fixed 32-bit instructions keep counted skips simple even though they are not compact in program memory.
