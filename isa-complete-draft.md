...
**Operand forms.** Binary register arithmetic (`ADD`, `SUB`, `MUL`, `DIV`, `MOD`) and binary bitwise logic (`AND`, `OR`, `XOR`) use `INSTRUCTION dst, src1, src2`. Both sources are read before `dst` is written, even if `dst` names one of the sources. `NOT` and the one-bit shifts use `INSTRUCTION dst, src1` because they have only one source. `ADD_CONST dst, src1, signed_imm16` uses a number encoded in the instruction instead of a second source register. The current counted condition form is `IF_* src1, src2, N`: it reads two registers, and `N` is an instruction count, **not a third register**. Whether conditions should instead have only two operands and skip exactly one instruction is under review. `GHOST N` and `REPEAT N` currently encode numeric counts; a register-controlled `GHOST` is under review.
