...
| Finish | `HALT` | Stops execution with a success status visible to the simulator or board diagnostic circuit. `GHOST 0` serves as a no-op, so there is no separate `NOP` instruction. |
